...
In order to validate our fluid-structure interaction solver, we realize here a benchmark, initially proposed by link:{biblio}#turek2006proposal[Turek and Hron], on the behavior of drag and lift forces in a specific model.
...
The reference results have been obtained with a time step $$\Delta t=0.05$$. When we compare them to our results, with the same step and $$BDF2$$, we observe that they are in accordance with reference ones.

With a $$\Delta t$$ smaller, a discrepancy is observe, in particular for the drag force. It can also be seen with the same time step, but with a higher $$BDF$$ ( in this case $$BDF3$$ ). This suggests that the couple $$\Delta t=0.05$$ and $$BDF2$$ isn't enough accurate.

To conclude, a quick look at geometrical order let us see how its use can give good results without necessarily the need for refine more the mesh. 
