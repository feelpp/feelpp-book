...
latexmath:[\begin{array}{cccccccccc}
      ...
      ~ &
      ~ & 
      \left\downarrow\right.\pi_{ch}&
      ~ & 
      \left\downarrow\right.\pi^{\mathrm{curl}}_h&
      ~ &
      \left\downarrow\right.\pi^{\mathrm{div}}_h&
      ~ &
      \left\downarrow\right.\pi_{dh}&
      ~ \\
      \mathbb{R}& 
      \overset{id}{\longrightarrow}&
      U_h&
...
The diagram is commutative which means that we have the following properties:
