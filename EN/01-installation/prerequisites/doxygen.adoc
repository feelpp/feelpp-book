...
- Two or more single line comments (//) that have at least one extra slash or
  exclamation point following the comment marker also form a special comment:
...
Special comments can contain Doxygen tags that let you specify features of your
program. By default, comments refer to the next lexical element, for example:   
...
We can configure our cmake system to enable Doxygen to automatically generate
an API documentation when we run `make`   commands. To generate a documentation
for our simple simple project using doxygen, add to our CMakeLists.txt the
following:
