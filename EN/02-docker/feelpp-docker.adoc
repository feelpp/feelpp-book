...
Docker shines in continuous integration. It provides a large set of operating system to build upon and allows to check the software in various context.

The FCS takes advantage of Docker to build `feelpp-env` and `feelpp-libs` for several operating systems and with different compilers any time a commit in the Feel++ repository is done.

|===
| Operating system | version | `feelpp-env` Tags 

| Ubuntu | 16.10 | `ubuntu-16.10`,  `latest`
