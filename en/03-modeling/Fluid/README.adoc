...
An alternative model is the Stokes model. It is valid in the case of small Reynolds number. It corresponds to same formulation that Navier-Equation but without the convective term $$\left( \boldsymbol{u}_{f} \cdot \nabla_{\mathrm{x}} \right) \boldsymbol{u}_{f}$$ .
